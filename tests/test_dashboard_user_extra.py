...
def test_dashboard_user_index_requires_login(client):
...
def test_dashboard_user_invalid_table(client):
    response = client.get('/notatable')
    assert response.status_code in (302, 308, 400, 401, 404)
